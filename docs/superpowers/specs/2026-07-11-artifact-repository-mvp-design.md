...
文件存储，用于保存 Artifact Blob、Manifest 原始字节和 Signature。调用方在
逻辑层面不感知 MinIO 的 Bucket 结构或内容寻址对象路径，所有操作都通过逻辑
仓库和逻辑路径寻址；但下载默认通过预签名 URL 直接指向对象存储（见 6.2 节），
这意味着网络连通性层面调用方仍需要能够到达对象存储的外部端点，属于逻辑透明
但网络不透明。服务配置区分用于服务端对象 I/O 的 `MINIO_ENDPOINT` 和参与
SigV4 签名、必须由调用方可达的 `MINIO_PUBLIC_ENDPOINT`，两者必须路由到同一
MinIO 实例和 Bucket，不能在签名后改写 Host。未配置 Public Endpoint 时禁用
重定向下载并强制代理模式。若部署环境不满足直连前提，6.2 节提供服务端代理
下载作为替代。
...
Blob 使用全局 SHA-256 标识，记录大小、内部 MinIO Object Key、状态
...
查询实际外键引用完成。Blob 的状态和 Lease 用于在 PostgreSQL 与 MinIO 之间提供
...
GET 默认返回 `307 Temporary Redirect`，指向短期 MinIO 预签名 URL（默认有效期
15 分钟）。这一模式要求调用方的网络能够直连对象存储的外部可达端点；如果部署
环境中 MinIO 只对内网可达（例如 CI Runner 无法直接访问内部对象存储），可通过
`?redirect=false` 让服务端以流式方式代理下载，由 API 进程转发 `Range` 请求头
并保持状态码语义。重定向模式下的 Range 请求由对象存储原生处理。MVP 默认使用
307 重定向以降低 API 进程带宽压力，代理模式作为网络受限部署的显式选项。
预签名必须直接使用 `MINIO_PUBLIC_ENDPOINT` 参与 SigV4 计算；若未配置该值，
`redirect=true` 返回 `409 public-endpoint-unavailable`，默认自动退化为代理模式。
...
短期 MinIO 预签名下载 URL。这里『不直接访问 MinIO』专指 Gateway 不持有 MinIO
凭据、不知道 Bucket/Object Key，也不调用 MinIO API；Gateway 或最终客户端跟随
制品库签发的短期 HTTP URL 在网络层访问 MinIO 是允许的。网络不允许直连 MinIO
时，调用方可以要求 Resolve 返回带 `redirect=false` 的制品库代理下载 URL。
...
如果 MinIO 成功而 Artifact 数据库事务失败，只能留下不可见的 `ready` 无引用
...
- 清理无引用 Blob 时，先在 PostgreSQL 短事务中锁定 Blob，确认所有 Artifact、
  ...
  必须等待或重试，不能新增引用。
- 提交 `deleting` 状态后删除 MinIO 对象，成功后再按 generation 条件删除 Blob 行；
  删除失败保留 `deleting` 状态由 Job 重试。若 Cleaner 发现没有数据库行的旧孤儿
  对象，必须先插入 `deleting` Tombstone 再删除，防止并发上传复用该对象。

PostgreSQL 事务只负责取得删除权和 fencing，MinIO 删除是独立、可重试的外部步骤。
...
- Release 状态、Artifact 数量、平台坐标、跨 Repository 引用等确定性校验在创建
  ...
  完成为对应 `4xx`。
- PostgreSQL/MinIO 超时、连接中断和对象存储 `5xx` 属于可重试失败，Release 保持
...
- **`readyz` 直接反映 PostgreSQL/MinIO 可用性**：作为 K8s Readiness Probe 时，
...
- MinIO 集成测试：流式上传、UploadSession 续租、Checksum Deploy、去重、预签名
  下载、`deleting` Blob 竞态和 staging 清理。
...
11. EdgeCLI Update Gateway 使用只读 Token Resolve candidate，不持有 MinIO 凭据、
    Bucket 或 Object Key；按部署网络条件验证预签名重定向和 API 代理两种下载模式。
...
25. `MINIO_ENDPOINT` 使用集群内部地址、`MINIO_PUBLIC_ENDPOINT` 使用外部地址时，
    预签名 URL 的 Host 与签名一致且客户端可下载；未配置 Public Endpoint 时只能
    使用代理模式，不能返回包含内部 DNS 的 URL。
